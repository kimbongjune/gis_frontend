...
const PipeInformation: React.FC = () => {
    ...
    const handleUploadSubmit = async () => {
        ...
        try {
            ...
            /* 
            // DANGEROUS: Picking the first random store (e.g. previous shapefile) causes mismatches
            // if we upload a NEW file. Disabling this until we can reliably identify the "Main DB" store.
            try {
                const dsRes = await axios.get(`http://localhost:8081/api/geo/workspaces/${targetWorkspace}/datastores`);
                if (dsRes.data && Array.isArray(dsRes.data) && dsRes.data.length > 0) {
                    // targetStore = dsRes.data[0]; 
                }
            } catch (err) {
    ...
    return (
        <div className="flex gap-4 h-[calc(100vh-180px)] overflow-hidden">
            ...
            <div className="w-80 bg-white rounded-lg shadow border border-gray-200 flex flex-col p-4 shrink-0 h-full">
                <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2 border-b pb-2">
                    <Search size={20} /> 시설물 정보
                </h2>
                ...
                <div className="mt-4 bg-orange-50 p-3 rounded text-xs text-orange-800 border border-orange-100 shrink-0">
                    ...
                    현재 로컬 GeoServer(8080)와 연동되어 있습니다. 서버가 닫혀있으면 지도가 표시되지 않습니다.
                </div>
            </div>
